...
class MigrationMatrixService:
    ...
    def list_entries(self) -> list[MigrationMatrixEntry]:
        ...
        return [
            ...
            MigrationMatrixEntry("comments", "implemented", "task.commentitem.getlist + crm.timeline.comment.list", "comment/timeline APIs", ["tasks", "crm", "smart_processes"], base_mapping, "counts+relations+integrity", "planned", "cleanup preview only", "Comment author/task links are verified"),
            MigrationMatrixEntry("files", "partial", "disk.file.get/list", "disk.file.upload", ["comments", "crm", "tasks"], base_mapping, "files+integrity", "planned", "cleanup preview only", "Metadata/reference migrated; binary payload copy remains planned"),
